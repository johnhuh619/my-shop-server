...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<?>> handleAccessDeniedException(AccessDeniedException e) {
        return ResponseEntity.status(HttpStatus.FORBIDDEN)
                .body(ApiResponse.error("C003", "Access denied"));
    }

    @ExceptionHandler(AuthenticationException.class)
    public ResponseEntity<ApiResponse<?>> handleAuthenticationException(AuthenticationException e) {
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                .body(ApiResponse.error("C004", "Authentication required"));
    }
    ...
    public ResponseEntity<ApiResponse<?>> handleException(Exception e) {
        ...
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(ApiResponse.error("C001", "Internal server error"));
    }
    ...
    private HttpStatus resolveHttpStatus(ErrorCode code) {
        return switch (code) {
            case USER_NOT_FOUND, ORDER_NOT_FOUND, PAYMENT_NOT_FOUND,
                 ...
                 ORDER_ITEM_NOT_FOUND, DELIVERY_NOT_FOUND -> HttpStatus.NOT_FOUND;
            ...
            case INVALID_INPUT_VALUE, INVALID_ORDER_STATUS, INVALID_REFUND_AMOUNT,
                 ...
                 ORDER_ALREADY_PAID, ORDER_EXPIRED,
